...
%
%Outputs:
%   data - Voltage signal (white noise) with the given input power spectral density.
...
vrms_sd=dbm2volt(psd_adc_dbm,zin);%Voltage NSD, in V/sqrt(Hz)
amp=vrms_sd*10*sqrt(bw); %Voltage in the bandiwdth, given by the amount of samples of the reconstructed signal 

data=amp*(randn(1,npts));  %Generating the white noise, time domain signal
time=0:Ts:Ts*(npts-1);
